...
class TestCardProperties:
    def test_effective_power_base(self) -> None:
        card = Card(id="test", name="Test", archetype=Archetype.SHARED, card_type=CardType.CLAIM, power=3)
        assert card.effective_power == 3

    def test_effective_power_upgraded(self) -> None:
        card = Card(
            id="test", name="Test", archetype=Archetype.SHARED,
    ...
    def test_action_return_values(self) -> None:
        standard = Card(id="a", name="A", archetype=Archetype.SHARED, card_type=CardType.CLAIM, action_return=0)
        net_neutral = Card(id="b", name="B", archetype=Archetype.SHARED, card_type=CardType.CLAIM, action_return=1)
        net_positive = Card(id="c", name="C", archetype=Archetype.SHARED, card_type=CardType.CLAIM, action_return=2)
...
class TestDeck:
    def test_draw_from_full_deck(self) -> None:
        cards = [Card(id=f"c{i}", name=f"Card {i}", archetype=Archetype.SHARED, card_type=CardType.CLAIM) for i in range(5)]
    ...
    def test_draw_reshuffles_discard(self) -> None:
        ...
        cards = [Card(id="c0", name="Card 0", archetype=Archetype.SHARED, card_type=CardType.CLAIM)]
        discard = [Card(id=f"d{i}", name=f"Discard {i}", archetype=Archetype.SHARED, card_type=CardType.CLAIM) for i in range(3)]
    ...
    def test_add_to_top(self) -> None:
        existing = Card(id="existing", name="Existing", archetype=Archetype.SHARED, card_type=CardType.CLAIM)
        new = Card(id="new", name="New", archetype=Archetype.SHARED, card_type=CardType.CLAIM)
    ...
    def test_total_cards(self) -> None:
        cards = [Card(id="c0", name="Card 0", archetype=Archetype.SHARED, card_type=CardType.CLAIM)]
        discard = [Card(id="d0", name="Discard 0", archetype=Archetype.SHARED, card_type=CardType.CLAIM)]
        deck = Deck(cards=cards, discard=discard)
        assert deck.total_cards == 2

    def test_shuffle_is_deterministic(self) -> None:
        cards = [Card(id=f"c{i}", name=f"Card {i}", archetype=Archetype.SHARED, card_type=CardType.CLAIM) for i in range(10)]
