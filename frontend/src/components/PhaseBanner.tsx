 ...
 */
export default function PhaseBanner({ phase, onMidpoint, onComplete }: PhaseBannerProps) {
  ...
  const enterMs = isNormal ? 600 : 200;
  const holdMs = isNormal ? 600 : 300;
  const exitMs = isNormal ? 600 : 200;
  ...
  if (isNormal) {
    ...
    switch (stage) {
      ...
      case 'enter':
        ...
        opacity = 1;
        transition = `transform ${enterMs}ms ease-out`;
        break;
      case 'hold':
        ...
        break;
      case 'exit':
        // Animate out to right + fade
        transform = 'translate(0%, -50%)';
        opacity = 0;
        transition = `transform ${exitMs}ms ease-in, opacity ${exitMs}ms ease`;
        break;
    }
  } else {
